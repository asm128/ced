...
#pragma pack( push, 1 )
	...
	struct SColorBGRA {
		...
		constexpr					bool			operator !=		(const SColorBGRA& other)								const	noexcept	{ return b != other.b || g != other.g || r != other.r || a != other.a;																																}
		constexpr					SColorBGRA		operator *		(const SColorBGRA& color)								const	noexcept	{ return SColorBGRA{(uint8_t)::std::min(b * (uint16_t)color.b, 255)	, (uint8_t)::std::min(g * (uint16_t)color.g, 255)	,(uint8_t)::std::min(r * (uint16_t)color.r, 255), a};			}
		constexpr					SColorBGRA		operator +		(const SColorBGRA& color)								const	noexcept	{ return SColorBGRA{(uint8_t)::std::min(b + (uint16_t)color.b, 255)	, (uint8_t)::std::min(g + (uint16_t)color.g, 255)	,(uint8_t)::std::min(r + (uint16_t)color.r, 255), a};			}
