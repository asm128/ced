...
int													ssg::setupStars			(::ssg::SStars & stars, ::ced::SCoord2<uint32_t> targetSize)	{
...
static	int											shipGeometryBuildEngine	(::ced::SGeometryQuads & geometry)			{
	::ced::geometryBuildHelix(geometry, 8U, 8U, 0.15f, {0, 1.1f, 0}, {1, 1, 1});
	::ced::geometryBuildCylinder(geometry, 1U, 6U, .05f, {0, 1, 0}, {1, 1, 1});
	::ced::geometryBuildCylinder(geometry, 1U, 8U, .35f, {0, 1, 0}, {1, 1, 1});
	::ced::geometryBuildCylinder(geometry, 1U, 8U, .35f, {0, 1, 0}, {-1, 1, 1});
	return 0;
}

static	int											shipGeometryBuildWafer	(::ced::SGeometryQuads & geometry)			{
	::shipGeometryBuildEngine(geometry);
	::ced::geometryBuildCube(geometry, {1.2f, 0.5f, 1.2f});
...
static	int											shipGeometryBuildGun	(::ced::SGeometryQuads & geometry)						{
	...
	::ced::geometryBuildFigure0	(geometry, 2U, 8U, 1, {});
	::ced::geometryBuildCylinder(geometry, 2U, 8U, .25f, {0, 0}, {1, 1, 1});
	::ced::geometryBuildCylinder(geometry, 2U, 8U, .25f, {0, 0}, {-1, 1, 1});
	return 0;
}
static	int											shipGeometryBuildCannon	(::ced::SGeometryQuads & geometry)			{
	::shipGeometryBuildEngine(geometry);
	::ced::geometryBuildSphere	(geometry, 8U, 5U, .8f, {0, 0});
	::ced::geometryBuildTender	(geometry, 8U, 8U, .75f, {0, -.5, 0}, { .25, .8f, .25});
	::ced::geometryBuildTender	(geometry, 8U, 8U, .75f, {0, -.5, 0}, {-.25, .8f, .25});
	return 0;
}

//static	int											shipGeometryBuildSilo	(::ced::SGeometryQuads & geometry)			{ return 0; }
//static	int											shipGeometryBuildShield	(::ced::SGeometryQuads & geometry)			{ return 0; }
...
static	int											modelsSetup				(::ssg::SShipScene & scene)			{
	...
	::ced::geometryBuildCube	(scene.Geometry[4], {1, 1, 1});
	::ced::geometryBuildSphere	(scene.Geometry[4], 4U, 2U, 1, {0, 0});
	::ced::geometryBuildSphere	(scene.Geometry[5], 6U, 2U, 1, {0, 0});

	{
		...
		for(uint32_t iImage = 0; iImage < scene.Image.size(); ++iImage) {
			...
			for(uint32_t y = 0; y < image.Metrics.y; ++y) {// Generate noise color for planet texture
				bool													xAffect						= (y % 2);
				::ced::SColorFloat										lineColor					= baseColor[(iImage + (rand() % 4)) % (::std::size(baseColor) - 4)];
...
int													ssg::solarSystemSetupBackgroundImage	(::ced::SImage & backgroundImage, ::ced::SCoord2<uint32_t> windowSize)	{
...
int													ssg::solarSystemSetup	(::ssg::SSolarSystem & solarSystem, ::ced::SCoord2<uint32_t> windowSize)	{
	...
	solarSystem.Scene.Camera[CAMERA_MODE_PERSPECTIVE].Position.RotateZ(ced::MATH_PI * .25);
	return 0;
}
